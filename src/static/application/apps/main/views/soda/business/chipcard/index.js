...
class App extends Component {
    ...
    this.columns = [
      {
        ...
        render: (mobile, record) => {
          return <Link to={`/soda/business/chipcard/${record.chipcardId}`}>{mobile}</Link>
        }
      }, {
        title: '充值金额(元)',
        dataIndex: 'value',
  ...
  changeHistory ({...options}) {
    ...
    this.props.history.push(`/soda/business/recharges-chipcard?${querystring.stringify(query)}`)
  }
  pagination () {
    let self = this
