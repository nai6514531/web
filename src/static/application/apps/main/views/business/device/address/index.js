...
class App extends Component {
    ...
    this.columns = [
      {
        ...
        render: (record) => {
          return <span>
            <Link to={`/business/device/address/edit/${record.id}`}>修改</Link>
          </span>
        }
      }
    ]
  ...
  changeHistory(options) {
    ...
    this.props.history.push(`/business/device/address?${querystring.stringify(query)}`)
  }
  search() {
    ...
    return (<div>
      ...
      <div>
        ...
        <Button
          type='primary'　
          style={{ marginRight: 10, marginBottom: 10 }}
          onClick={() => { this.props.history.push(`/business/device/address/add`) }}>新增地点</Button>
      </div>
      <Table
        scroll={{ x: 980 }}
        style= {{ marginTop: 16 }}
