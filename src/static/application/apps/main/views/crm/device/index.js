    ...
    this.columns = [
      {
        ...
        render: (record) => {
          ...
          return  <Link to={`/crm/device/${record.id}?serials=${serials || ''}&keys=${keys || ''}`}>{record.serial}</Link>
        }
      },
      {
        title: '运营商名称',
        ...
        render: (text, record, index) => {
          ...
          return (
            <span>
              ...
              <Link to={`/crm/device/operation/${serial}?serials=${serials || ''}&keys=${keys || ''}`}>{'\u00A0'}|{'\u00A0'}操作详情</Link>
            </span>
          )
        }
      }
