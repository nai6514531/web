...
const gameService = {
  ...
  export: (data) => {
    ...
    if(!data) {
      ...
      url = `/game/games/export?offset=${data.offset || 0 }&limit=${data.limit || 10 }&id=${data.id || ''}&supplierId=${data.supplierId || ''}&status=${data.status || ''}&startedAt=${data.startedAt || ''}&endedAt=${data.endedAt || ''}&orderBy=${data.orderBy || 'created_at'}&asc=${data.asc || 0}`          
    }
    return request.get(url)
  },
  order: (data) => {
