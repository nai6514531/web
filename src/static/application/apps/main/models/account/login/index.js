...
export default {
  ...
  effects: {
    ...
    *smsCode ({ payload: { data} }, { put, call }) {
      ...
      yield put({ type: 'updateData', payload: { smsLoading: true, accountHelp, passwordHelp, captchaHelp, smsCodeHelp } })
      const result = yield call(commonService.sms, data)
      if( result.status === 'OK' ) {
        yield put({ type: 'updateData', payload: { startedAt: +new Date(), smsLoading: false, startedAt: +new Date(), smsLoading: false  } })
      } else {
        if (result.status === 'NOT_FOUND_ENTITY') {
          accountHelp = {
            help: result.message,
            className:'has-error'
          }
        ...
        yield put({ type: 'updateData', payload: { ...help, smsLoading: false } })
      }
    },
    ...
    *login ({
      ...
      let [ accountHelp, passwordHelp, captchaHelp, smsCodeHelp ] = [ null, null, null ]
      const data = yield call(commonService.login, _.omit(payload.data, 'initPassword', 'checked', 'showSmsCode'))
      if(data.status == 'OK') {
        //登录成功后存储账户密码token等
