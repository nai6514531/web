...
export default {
  ...
  effects: {
    *getAssignedPermission({ payload: { data } }, { call, put, select }) {
      ...
      if( result.status == 'OK') {
        const data = yield select(state => state.assignPermissions.orignData)
    ...
    *cancel({ payload }, { call, put, select }) {
      const orignData = yield select(state => state.assignPermissions.orignData)
      yield put({
        type: 'updateData',
        payload: {
    ...
    *changeCheckedList({ payload: { data: { id, checkedList } } }, { call, put, select }) {
      const baseData = yield select(state => state.assignPermissions.baseData)
    ...
    *assign({ payload: { data: { id, roleId } } }, { call, put, select }) {
      const orignData = yield select(state => state.assignPermissions.orignData)
      const baseData = yield select(state => state.assignPermissions.baseData)
      const assignedPermission = yield select(state => state.assignPermissions.assignedPermission)
      const len = baseData.length
